...
app.post("/problems", (req, res) => {
  ...
  if (!userId) {
    return res
      .status(400)
      .json({ message: "Foydalanuvchi ID kiritilishi kerak!" });
  }
...
app.post("/all", (req, res) => {
  ...
  if (!user || user.type !== "admin") {
    return res
      .status(403)
      .json({ message: "Faqat admin barcha so'rovlarni ko'rishi mumkin!" });
  }
...
app.post("/reply", (req, res) => {
  ...
  if (!user || user.type !== "admin") {
    return res
      .status(403)
      .json({ message: "Faqat admin javob yuborishi mumkin!" });
  }
